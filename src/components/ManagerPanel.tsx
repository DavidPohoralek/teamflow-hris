...
export default function ManagerPanel({ onClose }: ManagerPanelProps) {
  ...
  const tabs = [
    { id: 'employees' as const, label: 'Zaměstnanci', icon: '👥' },
    { id: 'work-types' as const, label: 'Typy práce', icon: '🏷️' },
    { id: 'requests' as const, label: 'Žádosti', icon: '📋' },
    { id: 'settings' as const, label: 'Nastavení', icon: '⚙️' },
  ];
    ...
    <div className="fixed inset-0 z-50 flex bg-white">
      ...
      <div className="w-56 flex-shrink-0 bg-gray-900 flex flex-col">
        <div className="px-4 py-5 border-b border-gray-700">
          <h1 className="text-white font-semibold text-base leading-tight">Správa systému</h1>
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {['Jméno', 'PIN', 'Poměr', 'Oddělení', 'Pozice', 'Štítky', 'Cíl. hodiny', 'Aktivní', 'Akce'].map((h) => (
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      ...
                      {deleteConfirm === emp.id ? (
                        <span className="text-sm">
                          <button onClick={() => handleDelete(emp.id)} className="text-red-600 hover:text-red-800 mr-1">Smazat?</button>
                          <button onClick={() => setDeleteConfirm(null)} className="text-gray-400 hover:text-gray-600">Zrušit</button>
                        </span>
                      ) : (
...
function EmployeeForm({ employee, existingPins, onClose, onSaved }: EmployeeFormProps) {
  ...
  const validate = () => {
    ...
    if (!form.pin) {
      e.pin = 'PIN je povinný';
    } else if (!/^\d{4,8}$/.test(form.pin)) {
      e.pin = 'PIN musí mít 4–8 číslic';
    } else if (existingPins.includes(form.pin)) {
      e.pin = 'Tento PIN je již obsazen';
    }
    return e;
  };
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="font-semibold text-gray-900">{employee ? 'Upravit zaměstnance' : 'Přidat zaměstnance'}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600">✕</button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <FormField label="Jméno *" error={errors.name}>
            <input className={inputCls(errors.name)} value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="Jan Novák" />
          </FormField>
          <FormField label="PIN * (4–8 číslic)" error={errors.pin}>
          ...
          <FormField label="Pracovní poměr">
            <select
              className={inputCls()}
          ...
          <div className="flex gap-3 pt-2 justify-end">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border rounded-lg hover:bg-gray-50">Zrušit</button>
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
              {saving ? 'Ukládám…' : employee ? 'Uložit' : 'Přidat'}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="p-6">
      ...
      {!loading && !error && (
        <div className="space-y-2">
          {workTypes.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-8">Žádné typy práce</p>
          ) : (
            workTypes.map((wt) => (
              <div key={wt.id} className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-lg hover:border-gray-300 transition-colors">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${CATEGORY_COLORS[wt.category]}`}>
                  {CATEGORY_LABELS[wt.category]}
                </span>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => { setEditingWT(wt); setShowForm(true); }} className="text-blue-600 hover:text-blue-800 text-sm">Upravit</button>
                  {deleteConfirm === wt.id ? (
                    <>
                      <button onClick={() => handleDelete(wt.id)} className="text-red-600 hover:text-red-800 text-sm">Smazat?</button>
                      <button onClick={() => setDeleteConfirm(null)} className="text-gray-400 hover:text-gray-600 text-sm">Zrušit</button>
                    </>
                  ) : (
                    <button onClick={() => setDeleteConfirm(wt.id)} className="text-red-500 hover:text-red-700 text-sm">Smazat</button>
                  )}
                </div>
              </div>
            ))
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="font-semibold text-gray-900">{workType ? 'Upravit typ práce' : 'Přidat typ práce'}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600">✕</button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <FormField label="Název *" error={nameError}>
  ...
  return (
    <div className="mb-6 bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50">
        <h3 className="text-sm font-semibold text-slate-700">Přehled dovolených — kdo kdy chybí</h3>
      ...
      {loading ? (
        <div className="py-8 text-center text-sm text-slate-400">Načítám…</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-800 text-slate-300">
                <th className="sticky left-0 bg-slate-800 px-3 py-2 text-left font-medium w-28 whitespace-nowrap">Zaměstnanec</th>
                    ...
                    {days.map((d) => {
                      ...
                      return (
                        <td key={d} className={`px-0.5 py-1 text-center ${wd >= 5 ? 'bg-blue-50/40' : ''}`}>
                          {st === 'approved' && <span className="block w-5 h-5 rounded bg-emerald-400 mx-auto" title="Schválena" />}
                          {st === 'pending' && <span className="block w-5 h-5 rounded bg-amber-300 mx-auto" title="Čeká" />}
                        </td>
                      );
                    })}
                  </tr>
          ...
          <div className="flex gap-4 px-4 py-2 border-t border-slate-100 text-xs text-slate-500">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-emerald-400" />Schválena</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-300" />Čeká na schválení</span>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        ...
        >
          🏖️ {showVacationOverview ? 'Skrýt přehled dovolených' : 'Přehled dovolených'}
        </button>
      </div>
      ...
      {!loading && !error && (
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {['Zaměstnanec', 'Typ', 'Od', 'Do', 'Hodiny', 'Poznámka', 'Datum podání', ...(subTab === 'pending' ? ['Akce'] : [])].map((h) => (
...
function SettingsTab() {
  ...
  const handlePasswordSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPwdMsg({ type: 'success', text: 'Heslo bylo úspěšně změněno.' });
  ...
  return (
    <div className="p-6 space-y-8 max-w-2xl">
      ...
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-4">Manažerské heslo</h3>
        <form onSubmit={handlePasswordSave} className="space-y-4">
          <FormField label="Aktuální heslo">
            <input
              type="password"
              className={inputCls()}
          ...
          <FormField label="Potvrdit nové heslo">
            <input
              type="password"
              className={inputCls()}
          ...
          <div className="pt-1">
            <button type="submit" disabled={savingPwd} className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 disabled:opacity-50 font-medium">
              {savingPwd ? 'Ukládám…' : 'Uložit heslo'}
            </button>
          </div>
        </form>
      </section>
      ...
      {/* Company Info */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-4">Informace o firmě</h3>
        <FormField label="Název firmy">
          <input className={inputCls() + ' bg-gray-50'} disabled value="" placeholder="(načítá se…)" />
        </FormField>
        <p className="text-xs text-gray-400 mt-2">Název firmy je nyní jen pro čtení.</p>
      </section>

      {/* Attendance */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-4">Docházka</h3>
        <label className="flex items-center gap-3 cursor-pointer select-none">
          ...
          <span className="text-sm text-gray-700">Kiosk povolen</span>
        </label>
      </section>

      {/* Planning features */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Plánování směn</h3>
        <p className="text-xs text-gray-400 mb-4">Volitelné funkce pro firmy s víkendovým provozem</p>
        <ToggleSetting
          label="Logika sobot"
          description="Sledovat, zda zaměstnanci mohou pracovat v sobotu a kolik sobot za měsíc. Vhodné pro maloobchod a provoz o víkendech."
          settingKey="saturday_logic_enabled"
        />
        <div className="mt-4">
          <ToggleSetting
            label="Víkendový provoz"
            description="Firma má otevřeno i o víkendech. Víkendové buňky v gridu budou zvýrazněny jako pracovní dny. Směny lze přidávat o víkendech vždy bez ohledu na toto nastavení."
      ...
      {/* Bonus settings */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Bonusy — nastavení</h3>
        <p className="text-xs text-gray-400 mb-4">Příplatky pro výpočet bonusových hodin v exportu CSV</p>
        <div className="space-y-4">
          <NumberSetting label="Příplatek za sobotu (%)" description="% navíc za hodiny v sobotu. Např. 10 = za 8h soboty → 0,8h bonus." settingKey="bonus_saturday_pct" defaultValue={10} />
          <NumberSetting label="Přesčas — práh (h/měsíc)" description="Od kolika hodin měsíčně se počítá přesčas. 0 = přesčas se nepočítá." settingKey="bonus_overtime_threshold" defaultValue={0} />
          <NumberSetting label="Příplatek za přesčas (%)" description="% navíc za přesčasové hodiny." settingKey="bonus_overtime_pct" defaultValue={25} />
        </div>
      </section>

      {/* Provozní doba */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Provozní doba</h3>
        <p className="text-xs text-gray-400 mb-4">Nastavte otevírací hodiny pro každý den v týdnu</p>
        <OperatingHoursSetting />
      </section>

      {/* Zavřené dny */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Státní svátky / Zavřené dny</h3>
        <p className="text-xs text-gray-400 mb-4">Konkrétní dny, kdy je firma zavřená (formát: RRRR-MM-DD)</p>
        <ClosedDatesSetting />
      </section>

      {/* Absence a benefity */}
      <section className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Absence a benefity</h3>
        <p className="text-xs text-gray-400 mb-4">Konfigurace nemocenské a benefitů</p>
        <div className="space-y-6">
          <div>
            <p className="text-sm font-medium text-gray-700 mb-2">Nemocenská</p>
            <NumberSetting label="% z denního fondu hodin" description="Např. 60 = nemocenská = 60 % z 8h = 4,8h" settingKey="sick_leave_pct" defaultValue={60} />
          </div>
          <div>
            <p className="text-sm font-medium text-gray-700 mb-3">Benefity — konfigurace</p>
            <BenefitsSetting />
          </div>
        </div>
      </section>
...
const WEEKDAY_KEYS: { key: string; label: string }[] = [
  { key: 'hours_mon', label: 'Po' },
  { key: 'hours_tue', label: 'Út' },
  { key: 'hours_wed', label: 'St' },
  { key: 'hours_thu', label: 'Čt' },
  { key: 'hours_fri', label: 'Pá' },
  { key: 'hours_sat', label: 'So' },
  { key: 'hours_sun', label: 'Ne' },
];
    ...
    <div className="space-y-2">
        ...
        return (
          <div key={key} className="flex items-center gap-3 py-1.5 border-b border-gray-100 last:border-0">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer">
              ...
              <span className="text-sm text-gray-600">Otevřeno</span>
            </label>
            {isOpen && (
      ...
      <div className="pt-2">
        <button onClick={handleSave} disabled={saving}
          ...
          {saved ? '✓ Uloženo' : saving ? '…' : 'Uložit provozní dobu'}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        {saved && <span className="text-emerald-600 text-sm">✓ Uloženo</span>}
      </div>
    </div>
  );
}
...
const BENEFIT_ROWS: BenefitRow[] = [
  { label: 'Darování krve', hoursKey: 'benefit_blood_hours', maxKey: 'benefit_blood_max', defaultHours: 8, defaultMax: 1 },
  { label: 'Angličtina', hoursKey: 'benefit_english_hours', maxKey: 'benefit_english_max', defaultHours: -1, defaultMax: 4 },
  { label: 'Cvičení', hoursKey: 'benefit_gym_hours', maxKey: 'benefit_gym_max', defaultHours: -1, defaultMax: 4 },
];
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2 text-xs font-semibold text-gray-500 uppercase tracking-wider pb-1 border-b border-gray-100">
        <span>Benefit</span>
        <span>Efekt hodin</span>
        <span>Max. za měsíc</span>
      </div>
      ...
      <div className="pt-2">
        <button onClick={handleSave} disabled={saving}
          className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${saved ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'} disabled:opacity-50`}>
          {saved ? '✓ Uloženo' : saving ? '…' : 'Uložit benefity'}
        </button>
      </div>
    </div>
  );
