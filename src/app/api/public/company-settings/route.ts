...
export async function GET(req: NextRequest) {
  ...
  const { data: row } = await supabase
    ...
    .select('kiosk_enabled, ui_theme, saturday_logic_enabled, weekend_open, extra_settings')
    .eq('organization_id', orgId)
    .maybeSingle();

  const r = (row ?? {}) as { kiosk_enabled?: boolean | null; ui_theme?: string | null; saturday_logic_enabled?: boolean | null; weekend_open?: boolean | null; extra_settings?: Record<string, unknown> | null };
